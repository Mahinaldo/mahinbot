...
const defaultComponents = memoizeMarkdownComponents({
  ...
  th: ({ className, ...props }) => (
    <th className={cn("px-4 py-2 text-left font-bold first:rounded-tl-lg last:rounded-tr-lg [&[align=center]]:text-center [&[align=right]]:text-right", className)} {...props} />
  ),
  td: ({ className, ...props }) => (
  ...
  pre: ({ className, ...props }) => (
    <pre className={cn("overflow-x-auto rounded-b-lg p-4 text-white", className)} {...props} />
  ),
  code: function Code({ className, ...props }) {
    const isCodeBlock = useIsMarkdownCodeBlock();
    return (
      <code
        className={cn(!isCodeBlock && "rounded border font-semibold", className)}
        {...props}
      />
    );
  },
